...
- Strict finance mode is the default. A structured `analysisPlan` with any missing, disabled, incompatible, or unbridged metric returns `unsupported` with empty SQL; it never downgrades to estimate or enters historical/template/generic fallback.
- Estimate finance mode is used when the user explicitly asks for rough decision support, such as `估算`, `大概`, `大致`, `粗算`, `趋势`, or `决策参考`. The legacy unstructured path may also downgrade when strict coverage is absent and semantically matching historical evidence exists; structured plans never use this downgrade. Estimate output must be labeled as non-accounting guidance.
- Draft metrics remain documentation and review material only. They are never generation authority.
- Diagnostic exception: only the exact environment value `ERP_SQL_DIAGNOSTIC_BYPASS_COMPOSITE_CAPABILITY=true` and a qualifying finance composite plan may inspect existing `approved`/`draft` definitions whose `kind='atomic_metric'`. The composer bypasses only draft/disabled approval gates; it emits `diagnostic_unapproved_metric_bypass` only when one of those definitions is actually selected, and any successful result is forced to `semanticStatus=estimate`. All structure, access, SQL, schema, and runtime guards remain active. In particular, gross-margin definitions without reviewed `documentPreaggregationKeys` still fail closed. This path neither approves a metric nor mutates the metric catalog or business data.
...
- Estimate generation remains available only for explicit rough/decision-support requests, or legacy unstructured questions with semantically matching historical evidence and valid schema. It cannot replace missing coverage in a structured plan.
...
Atomic metrics use `definition_json.kind = "atomic_metric"`. The Mastra ERP SQL toolchain can first build an `analysisPlan` from business-analysis questions with rules first and JSON-only LLM fallback for still-unmatched analysis phrasing, then compose strict SQL only when every requested metric has `status = 'approved'`, `kind = 'atomic_metric'`, and compatible grain or shared `joinKeys`. Missing, disabled, or incompatible atomic metrics make the structured plan unsupported.
...
Approved composite finance metrics may run before atomic composition when they exactly cover a known strict finance question and provide reviewed `representative_sql`. Every declared composite member is resolved again against the current approved atomic catalog and must be atomic, enabled, non-circular, and fully available; membership names alone are not authority. The SQL still goes through `SqlGuardService` with the approved metric reference.
