...
flowchart LR
  ...
  D["ERP_SQL_ACCESS_POLICY_JSON"] --> C
...
- 数据范围暂由部署配置按 identity user id 映射；缺失、解析失败或字段不完整均拒绝。
- Company 永远是具体非空列表，每个 `Erp.*` source 都改写为带 Company predicate 的派生表。
...
在这些依赖完成前，使用 `ERP_SQL_ACCESS_POLICY_JSON` 做最小可落地映射。未知字段、未知模块或缺映射保持 fail closed；不设置 permissive 默认值。
