...
输入中的 `package_item_order` 是产品族发现顺序，不冒充 extraction item index。只有唯一名称对应的结构化 item 产品号，或单产品包中的唯一六位产品号候选，才作为精确 item 证据；多产品包无法分配的文档级号码明确留作 blocker。输出不含客户、联系人、地址、金额、价格或 BOM 明细。
