...
- `ERP_RESULT_NARRATOR_EXTERNAL_ENABLED` + `ERP_RESULT_NARRATOR_EXTERNAL_TRUSTED`：双开关；默认不外发，开启后发送聚合/DLP 后数据。另设 `ERP_RESULT_NARRATOR_EXTERNAL_RAW_ROWS_ENABLED=true` 时，才向已信任的外部模型发送当前授权查询的字段名和最多 50 行结果；LLM 审计日志仍按默认保护策略保存摘要。
