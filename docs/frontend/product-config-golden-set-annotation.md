...
配置字段允许从空状态点击“＋ 添加配置字段”逐项新增，默认字段名使用当前任务内唯一的 `configuration_field_N`，值、单位、选项和证据均保持未填写；每项可单独“移除配置字段”。新增、编辑和删除都更新完整 `configuration_fields` 数组并进入同一草稿 autosave，不改动现有 ERP mapping 对账与持久化流程。

A/B 席位由后端按个人账号的 `product-config-agent.golden-set.annotate-a` / `.annotate-b` 权限派位，裁决账号使用独立 `.adjudicate` 权限；标注与裁决职责互斥。操作者不能在页面切换席位，也不能读取对方草稿或提交；裁决账号只在双方都已提交且答案不同后，才能从差异复核接口看到 A/B 答案。不要用假用户、改请求头或手工标记 A/B 来替代真实权限验证。
