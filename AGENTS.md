...
- 手机端优先按真实 App 页面设计，不要把桌面卡片、表格按钮和三栏布局简单缩窄。聊天、列表、工作台类页面应优先使用全屏主视图、底部固定输入区、侧滑抽屉或底部 sheet。
- 尺寸必须响应式：优先用 `dvh` / `dvw`、`clamp()`、`min()`、`max()` 和 `env(safe-area-inset-*)`；避免用固定 `320px`、`350px` 之类假设某一款手机。固定上限只用于大屏居中容器，且要配合视口计算。
- 抽屉/侧栏用 CSS 变量统一宽度，例如 `--drawer-width`，主内容位移、菜单按钮位置和抽屉宽度必须共用同一个变量，避免不同机型错位。
- 隐藏的抽屉不要保留阴影、边框或可点击区域；阴影只放在展开态，关闭态要 `pointer-events: none` 并完全移出视口，避免左侧漏出灰边。
- 输入框、搜索框、底部操作栏必须设置 `box-sizing: border-box`，宽度使用 `calc(100dvw - safe-area/边距)` 或父容器内 `width: 100%`，防止 padding 把右侧顶出屏幕。
- 手机端触控目标不小于约 38-44px，但不要把辅助控件做成桌面大按钮；搜索框、主菜单、分页等控件应使用胶囊/图标/紧凑按钮，避免突兀。
- 长按、滑动等移动交互要有兜底：长按菜单至少支持右键/上下文菜单；滑出抽屉后应支持点击主内容或反向滑动关闭。
- 每次改手机 UI 后至少检查 360px、390px、430px 三档宽度；重点看是否右侧抵边、文本溢出、底部输入区遮挡、抽屉阴影残留和安全区间距。
