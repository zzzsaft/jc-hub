...
- Wrapped drawer actions so the Task 5 commit compiles against the detached baseline Panel callback contract as well as the current shared-worktree contract.
- Restricted numeric/string compatibility to the `order` dimension with a canonical unsigned digit filter and a safe integer/bigint result. String identifiers retain leading zeros and sign syntax.
- Moved result-range assertion behind executor success determination, preserving executor errors and partial diagnostic rows from reclassification.
