...
export function ConfigFieldsForm({ value, items, evidence, onChange }: Props) {
  ...
  return <fieldset>
    ...
    {value.map((field, index) => <article className="full-review-item" key={field.field_key}>
      ...
      <button type="button" className="full-review-danger" onClick={() => onChange(removeConfigurationField(value, field.field_key))}>移除配置字段</button>
    </article>)}
