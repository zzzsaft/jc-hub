...
test("auto-promote dry-run routes batch2 templates and references with compact SQL redaction", async () => {
  ...
  assert.equal(report.summary.appliedTemplateDrafts, 8);
  assert.equal(report.summary.downgradedReferences, 2);
  assert.equal(report.summary.registeredMetricDrafts, 0);
  assert.equal(report.summary.failed, 0);
  assert.deepEqual(report.appliedTemplates.map((item) => item.familyId).sort(), ["family_006", "family_014", "family_027", "family_031", "family_038", "family_086", "family_089", "family_092"]);
  assert.deepEqual(report.downgradedReferences.map((item) => item.familyId).sort(), ["family_008", "family_080"]);
...
test("auto-promote apply writes only draft templates and reference rows", async () => {
  ...
  assert.equal(repo.references.length, 2);
  assert.equal(repo.metrics.length, 0);
  assert.equal(report.verification?.summary.templateDraftFound, 8);
  for (const template of repo.templates) {
    assert.match(template.sqlTemplate, /^\s*SELECT\s+TOP\s+100\b/iu);
...
const TABLE_COLUMNS: Record<string, string[]> = {
  ...
  "Erp.ECORev": ["Company", "PartNum", "PartDescription", "RevisionNum", "RevDescription", "Approved", "ApprovedDate"],
  "Erp.ECOMtl": ["Company", "PartNum", "RevisionNum", "MtlSeq", "MtlPartNum", "QtyPer", "FixedQty", "RelatedOperation"],
  "Erp.JobOper": ["Company", "JobNum", "AssemblySeq", "OprSeq", "OpCode", "OpDesc", "PartNum", "JobComplete", "OpComplete", "ProdComplete", "QtyCompleted", "StartDate", "DueDate", "ActProdHours", "ActSetupHours"],
};
