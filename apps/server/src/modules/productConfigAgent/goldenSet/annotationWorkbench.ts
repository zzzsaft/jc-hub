...
function evidenceFor(packet: PackagePacket | ErpPacket) {
  ...
  const erp = packet.layer === "erp_identity" ? (packet.prediction as ErpPacket["prediction"]).top_candidates.map((v, i) => ({ evidence_id: `erp-${i + 1}`, company: v.company, part_num: v.part_num, product_name: v.erp_product_name, prod_code: v.prod_code, has_bom: v.has_bom, order_evidence: v.erp_order_num ? "有明确订单证据" : "无明确订单证据" })) : [];
  return { evidence_id: `evidence:${packet.sample_id}`, source: safe, erp_candidates: erp, note: "仅供人工判断；系统预测与另一位标注员答案均不可见。" };
}
...
export const annotationWorkbench = {
  list(userId: string, layer: Layer, page = 1, pageSize = 20) { const slot = userSlot(userId); const store = readStore(); const all = packets(layer); const items = all.slice((page - 1) * pageSize, page * pageSize).map(p => ({ sample_id: p.sample_id, status: store.submitted[key(layer, p.sample_id, slot)] ? "已提交" : store.drafts[key(layer, p.sample_id, slot)] ? "草稿" : "待处理", strata: p.strata })); return { items, page, pageSize, total: all.length, progress: { total: all.length, submitted: items.filter(v => v.status === "已提交").length } }; },
  ...
  adjudicationQueue() { const store = readStore(); return (["product_package", "erp_identity"] as Layer[]).flatMap(layer => packets(layer).filter(p => store.submitted[key(layer,p.sample_id,"annotator_a")] && store.submitted[key(layer,p.sample_id,"annotator_b")] && !store.adjudications[key(layer,p.sample_id)]).map(p => ({ sample_id: p.sample_id, layer, annotator_a: store.submitted[key(layer,p.sample_id,"annotator_a")], annotator_b: store.submitted[key(layer,p.sample_id,"annotator_b")], evidence: evidenceFor(p) }))); },
