...
export class SqlGuardService {
  ...
  private validateFinanceRules(
    ...
    const hasReference = (options.references ?? []).some((reference) =>
      reference.sourceType === "family" || Boolean(reference.familyId || reference.exampleSql || reference.sqlPreview)
    );
    if (!hasReference) {
      errors.push("Finance SQL must be grounded by a historical SQL family or template reference.");
    }
