...
const userLookupWhere = (userId: string, wecomUserId?: string | null): Prisma.UserWhereInput => ({
  OR: [
    { id: userId },
...
export const upsertAuthenticatedUser = async (authUser: AuthServiceUser): Promise<AuthenticatedUser> => {
  const user = await prisma.$transaction(async (tx) => {
    const existingUser = await tx.user.findFirst({
      where: userLookupWhere(authUser.userId, authUser.wecomUserId),
    ...
    const user = existingUser
      ...
      : await tx.user.create({
          data: {
            ...(authUser.wecomUserId ? {} : { id: authUser.userId }),
...
export const resolveLocalUser = async (localUser: NonNullable<ReturnType<typeof verifyLocalToken>>) => {
  const existingUser = await prisma.user.findFirst({
    where: userLookupWhere(localUser.userId, localUser.wecomUserId),
    include: { userRoles: { include: { role: true } } }
  });
