...
import { canTryAuth } from "../../modules/auth/rate-limit.js";
import {
  createWecomWebhookEvent,
  decryptWecomContactCallbackBody,
  finishWecomWebhookEvent,
  handleWecomContactCallback,
  wecomWebhookPayload,
  verifyWecomContactCallbackUrl
...
const router = Router();
const callbackBodyParser = express.text({ type: ["text/*", "application/xml", "*/xml"] });
...
const callbackClientId = (req: Request) => normalizeWecomClientId(req.query.clientId ?? DEFAULT_WECOM_CLIENT_ID);

router.get("/auth/wecom/contact-callback", (req, res, next) => {
  try {
    res.type("text/plain").send(verifyWecomContactCallbackUrl(callbackClientId(req), req.query));
  } catch (error) {
    next(error);
  }
});

router.post("/auth/wecom/contact-callback", callbackBodyParser, async (req, res, next) => {
  let webhookEventId: string | null = null;
  try {
    const clientId = callbackClientId(req);
    const webhookEvent = await createWecomWebhookEvent(clientId, {
      query: req.query,
      headers: req.headers,
      rawBody: req.body
    });
    webhookEventId = webhookEvent.id;
    const messageXml = decryptWecomContactCallbackBody(clientId, req.query, req.body);
    const payload = wecomWebhookPayload(messageXml);
    handleWecomContactCallback(clientId, messageXml).then((status) => {
      if (!webhookEventId) return;
      return finishWecomWebhookEvent(webhookEventId, status, payload);
    }).catch((error) => {
      if (webhookEventId) {
        void finishWecomWebhookEvent(webhookEventId, "failed", payload, error).catch((logError) => {
    ...
    if (webhookEventId) {
      void finishWecomWebhookEvent(webhookEventId, "failed", undefined, error).catch((logError) => {
        logger.error(logError instanceof Error ? logError.stack || logError.message : String(logError));
      });
    }
    next(error);
