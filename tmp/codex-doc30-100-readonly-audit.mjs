...
function contentText(blocksJson, llmText) {
  ...
  if (blocksJson && typeof blocksJson === "object" && typeof blocksJson.llm_text === "string" && blocksJson.llm_text.trim()) {
    return text(blocksJson.llm_text);
  }
  if (blocksJson && typeof blocksJson === "object" && Array.isArray(blocksJson.blocks)) {
    return text(blocksJson.blocks.map(blockText).join("\n"));
  }
...
function pickLines(value) {
  ...
  const keep = [];
  const patterns = /(合同|订单|产品|客户|国家|日期|交货|数量|名称|型号|规格|材料|材质|硬度|层|上模|下模|模唇|侧板|加热|进料|出料|流道|宽度|厚度|温度|压力|电压|功率|接线|安装|传感器|热电偶|备注|编号)/;
  lines.forEach((line, index) => {
    if (index < 18 || patterns.test(line)) keep.push({ line: index + 1, text: line.slice(0, 260) });
  });
  return keep.slice(0, 160);
}
...
try {
  ...
  const outJson = path.resolve("tmp/codex-doc30-100-readonly-audit-data.json");
  const outMd = path.resolve("tmp/codex-doc30-100-readonly-audit-report.md");
  fs.writeFileSync(outJson, json(report));
  fs.writeFileSync(outMd, [
    "# Codex Document 30-100 Readonly Audit",
    "",
    ...report.selected.flatMap((doc) => [
      ...
      "### Normalized",
      "```json",
      json(doc.normalizedSummary).slice(0, 20000),
      "```",
      "",
      "### Candidate",
      "```json",
      json(doc.candidates).slice(0, 14000),
      "```",
      "",
      "### Lines",
