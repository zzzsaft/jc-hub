...
export class SqlTemplateExecutionService {
  constructor(
    ...
    private readonly queryClient: QueryClient = getErpSqlQueryClient(),
  ) {}
  ...
  async execute(input: TemplateExecutionInput): Promise<TemplateExecutionResult> {
    ...
    if (!template.approved || template.approvalStatus !== "approved") return empty(template.sqlTemplate, "Template is not approved.");
    if (!template.guardPassed) return empty(template.sqlTemplate, "Template guard has not passed.");
    ...
    if (paramError) return empty(template.sqlTemplate, paramError);
